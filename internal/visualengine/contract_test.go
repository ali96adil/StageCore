...
func TestValidateCommandAcceptsCanonicalFoundationCommands(t *testing.T) {
	...
	cases := []struct {
		...
		{CapabilityPreload, `{"contract_version":1,"layer_id":"main","content_version_id":"version-1","content_hash":"` + hash + `","content_mode":"CROP","opacity":0.8,"transform":{"x":0.1,"y":-0.2,"scale_x":1.2,"scale_y":0.9,"rotation_degrees":12}}`},
		{CapabilityPreload, `{"contract_version":1,"layer_id":"fill","content_version_id":"version-2","content_hash":"` + hash + `","content_mode":"FILL"}`},
		{CapabilityPreload, `{"contract_version":1,"layer_id":"fit","content_version_id":"version-3","content_hash":"` + hash + `","content_mode":"FIT"}`},
	...
	for _, tc := range cases {
		t.Run(tc.capability+tc.params, func(t *testing.T) {
...
func TestValidateCommandRejectsAmbiguousOrUnsafeParameters(t *testing.T) {
	...
	cases := []struct {
		...
		{"invalid content mode", CapabilityPreload, `{"contract_version":1,"layer_id":"main","content_version_id":"version-1","content_hash":"` + hash + `","content_mode":"STRETCH"}`},
		{"lowercase content mode", CapabilityPreload, `{"contract_version":1,"layer_id":"main","content_version_id":"version-1","content_hash":"` + hash + `","content_mode":"fit"}`},
		{"null content mode", CapabilityPreload, `{"contract_version":1,"layer_id":"main","content_version_id":"version-1","content_hash":"` + hash + `","content_mode":null}`},
