...
func (s *Service) List(ctx context.Context, projectID string) (Inputs, error) {
	...
	for _, input := range manifest.Inputs {
		if input.Enabled { items = append(items, input) }
	}
...
func (s *Service) activeSimulation(ctx context.Context, projectID string) (domain.Session, error) {
	...
	if projectID == "" { return domain.Session{}, fmt.Errorf("%w: project is required", domain.ErrInvalidInput) }
	active, err := s.store.ActiveSessionForProject(ctx, projectID)
	if err != nil { return domain.Session{}, err }
	if active == nil || active.Type != domain.SessionSimulation || active.Status != domain.SessionActive || active.LifecycleState != domain.SessionLifecycleActive {
		return domain.Session{}, fmt.Errorf("%w: ACTIVE SIMULATION session is required", domain.ErrConflict)
	}
	return s.store.GetSessionFoundation(ctx, active.ID)
}
